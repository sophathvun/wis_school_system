...
const openPrintPreview = () => {
    if (gradePrintWindow && !gradePrintWindow.closed) {
        gradePrintWindow.focus();
        return;
    }

    gradePrintWindow = window.open("/grades/print", "grade-print", "noopener");
};
...
const openEditModal = (id) => {
    ...
    document.getElementById("grade_short_name").value =
...
form?.addEventListener("submit", async (event) => {
    ...
    const requiredFields = {
        ...
        grade_short_name: "Short name is required.",
    };
    ...
    if (Object.keys(clientErrors).length) {
        ...
        submitButton.textContent = document.getElementById("grade_id").value
            ? "Update"
            : "Create";
        return;
    }
    try {
        const response = await fetch("/grades/save", {
            method: "POST",
        ...
        try {
            ...
            throw new Error("Unable to save grade. Please try again.");
        }
        ...
        if (!response.ok || result.status !== "success")
            throw new Error(result.message || "Unable to save grade.");
        modal?.hide();
...
const deleteGrade = async (id) => {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(result.message || "Unable to delete grade.");
...
async function fetchGrades(page = 1, perPage = null) {
    ...
    try {
        const response = await fetch(
            `/grades/fetch?page=${page}&perPage=${size}&search=${encodeURIComponent(search.value)}&sortBy=${sortBy}&sortDir=${sortDir}`,
        );
        const result = await response.json();
        if (!response.ok)
...
document
    .getElementById("btnNewGrade")
    ?.addEventListener("click", openCreateModal);
document
    .getElementById("btnNewGradeMobile")
    ?.addEventListener("click", openCreateModal);
document
    .getElementById("btnPrintGrades")
