...
const routes = {
    fetchUrl: page?.dataset.fetchUrl || "",
    historyOptionsUrl: page?.dataset.historyOptionsUrl || "",
    optionsUrl: page?.dataset.optionsUrl || "",
...
const withdrawalReasons = (() => {
    try {
        return JSON.parse(page?.dataset.reasons || "[]");
    } catch {
        return [];
    }
})();

const initWorkflowStatusButtons = () => {
    const table = document.getElementById("withdrawal-table");
    if (!table || table.dataset.workflowStatusReady) return;
    table.dataset.workflowStatusReady = "1";
    const normalizePending = () =>
        table
            .querySelectorAll("td:nth-child(11) .badge:not(.workflow-status-button)")
            .forEach((badge) => {
    ...
    const convertPrincipalApproved = () =>
        table
            .querySelectorAll("td:nth-child(11) .badge:not(.workflow-status-button)")
            .forEach((badge) => {
    ...
    const syncWorkflowLabels = () =>
        table.querySelectorAll("tr").forEach(async (row) => {
            ...
            if (
                !["pending", "principal_approved"].includes(status) &&
                badge.classList.contains("workflow-status-button")
            ) {
    ...
    table.addEventListener(
        ...
        async (event) => {
            ...
            const status =
                ...
                (button.textContent.includes("Pending") ? "pending" : "principal_approved");
            const choice = window.Swal
                ...
                : { isConfirmed: window.confirm("Approve this withdrawal?"), value: "approve" };
            if (!choice.isConfirmed) return;
            let reason = "";
            if (choice.value === "reject" || choice.value === "cancel") {
                const reasonResult = window.Swal
                    ? await window.Swal.fire({
                          title:
                              choice.value === "reject"
                          ...
                          input: "textarea",
                          inputLabel:
                              choice.value === "reject" ? "Rejection reason" : "Cancellation reason",
                          inputValidator: (value) =>
                              !value?.trim() ? "A reason is required." : undefined,
                          showCancelButton: true,
                          confirmButtonText:
                              choice.value === "reject" ? "Reject Request" : "Cancel Request",
                          cancelButtonText: "Back",
                          reverseButtons: true,
                          icon: "warning",
                      })
                    : { isConfirmed: true, value: window.prompt("Reason") || "" };
...
const initWithdrawalDatePickers = () => {
    if (window.withdrawalDatePicker || window.editWithdrawalDatePicker) return;
    ...
    const formatWithdrawalIso = (date) =>
        date
    ...
    const setupWithdrawalDatePicker = (prefix) => {
        ...
        const hidden =
            document.getElementById(prefix.replace(/_/g, "-") === "withdrawal-date" ? "withdrawal-date" : "edit-withdrawal-date") ||
        ...
        const renderCalendar = () => {
            ...
            if (label) {
                label.textContent = cursor.toLocaleDateString("en-US", {
                    month: "long",
                    year: "numeric",
                });
            }
...
document.addEventListener("DOMContentLoaded", () => {
    initWorkflowStatusButtons();
    initWithdrawalDatePickers();
});


// Student withdrawal workflow migrated from Blade
