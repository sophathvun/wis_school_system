...
const loadOptions = async () => {
    const j = await (await fetch("/programs/options")).json();
    academic_year_id.innerHTML = j.academicYears
        .map((x) => `<option value="${x.id}">${x.academic_year}</option>`)
        .join("");
    education_level_id.innerHTML = j.levels
...
const openEdit = async (id) => {
    ...
    if (!row) return;
    await openCreate();
    program_id.value = row.id;
    academic_year_id.value = row.academic_year_id ?? "";
    education_level_id.value = row.education_level_id;
    program_name.value = row.program_name;
    program_code.value = row.program_code;
