...
const modal = new bootstrap.Modal(
    document.getElementById("academicTrackModal"),
);
let rows = [];
...
const reset = () => {
    ...
    document.getElementById("academicTrackModalTitle").textContent =
        "New Academic Track";
...
const edit = (id) => {
    ...
    document.getElementById("academic_track_name_en").value =
        item.name_en || "";
    document.getElementById("academic_track_name_kh").value =
        item.name_kh || "";
    document.getElementById("academic_track_code").value = item.code || "";
    document.getElementById("academic_track_grade_id").value =
        item.grade_id || "";
    document.getElementById("academic_track_stream_type").value =
        item.stream_type || "science";
    document.getElementById("academic_track_language").value =
        item.language || "khmer";
    document.getElementById("academic_track_status").value = String(
        item.status ? 1 : 0,
    );
    document.getElementById("academicTrackModalTitle").textContent =
        "Edit Academic Track";
...
const remove = async (id) => {
    if (
        !(
            await showConfirm(
                ...
                "Deactivate assigned tracks instead of deleting them.",
                "Delete",
                "Cancel",
            )
        ).isConfirmed
    ...
    if (!response.ok)
        return showError(
            "Unable to delete academic track",
            result.message || "Unable to delete academic track.",
        );
...
const fetchRows = async (page = 1, size = null) => {
    ...
    const result = await response.json();
    if (!response.ok)
        throw new Error(result.message || "Unable to load academic tracks.");
    rows = result.data || [];
    ...
    renderPagination(
        result,
        "academic-tracks-pagination-container",
        "academic-tracks-per-page",
        fetchRows,
    );
...
document.querySelectorAll(".table-sort-button").forEach((button) => {
    ...
    button.addEventListener("click", () => {
        ...
        currentSortDir =
            currentSortBy === selectedSort && currentSortDir === "asc"
                ? "desc"
                : "asc";
...
form?.addEventListener("submit", async (event) => {
    ...
    if (!response.ok)
        return showError(
            "Unable to save academic track",
            result.message || "Unable to save academic track.",
        );
    modal.hide();
...
fetchRows().catch((error) =>
    showError("Unable to load academic tracks", error.message),
);
