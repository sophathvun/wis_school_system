...
document.querySelectorAll("[data-branding-dropzone]").forEach((dropzone) => {
    ...
    const imageFileFromPasteEvent = (
        event,
        filename = "pasted-branding-image.png",
    ) => {
...
document.addEventListener("paste", (event) => {
    if (
        ...
        return;
    const activeDropzone = document.activeElement?.closest?.(
        "[data-branding-dropzone]",
    );
    ...
    event.preventDefault();
    const normalized = new File([file], "pasted-branding-image.png", {
