...
async function fetchClasses(page = 1, perPage = null) {
    ...
    try {
        ...
        if (mobileCards) mobileCards.innerHTML = classes.length ? classes.map((item, index) => {
            const active = Boolean(item.status);
            const statusToggle = window.statusToggleMarkup?.("class", item.id, active) || `<button type="button" class="status-toggle ${active ? "is-active" : ""}" data-status-toggle data-status-entity="class" data-status-id="${item.id}" data-status="${active ? 1 : 0}" aria-pressed="${active ? "true" : "false"}"><span class="status-toggle-label">${active ? "ON" : "OFF"}</span><span class="status-toggle-knob"></span></button>`;
            const locked = (Number(item.groups_count ?? 0) + Number(item.enrollments_count ?? 0) + Number(item.enrollment_history_count ?? 0) + Number(item.graduations_count ?? 0)) > 0;
            const deleteButton = locked ? `<button class="btn btn-outline-danger" disabled title="This class is linked to other data"><i class="ti ti-lock me-1"></i>Delete</button>` : `<button onclick="classesPage.deleteClass(${item.id})" class="btn btn-outline-danger"><i class="ti ti-trash me-1"></i>Delete</button>`;
            return `<article class="class-mobile-card"><span class="class-mobile-number">${String(offset + index + 1).padStart(2, "0")}</span><div class="class-mobile-status">${statusToggle}</div><div class="class-mobile-main-row"><strong class="class-mobile-name">${escapeHtml(item.class_name)}</strong><span>Order ${escapeHtml(item.class_order ?? "-")}</span><span>${locked ? "Linked to other data" : "Ready to manage"}</span></div><div class="class-mobile-actions"><button onclick="classesPage.openEditModal(${item.id})" class="btn btn-primary"><i class="ti ti-pencil me-1"></i>Edit</button>${deleteButton}</div></article>`;
        }).join("") : `<div class="class-mobile-empty">No classes found.</div>`;
        table.innerHTML = classes.length
            ? classes
                  .map(
                      (item, index) =>
                          `<tr><td>${offset + index + 1}</td><td>${item.class_name}</td><td>${item.class_order ?? ""}</td><td>${window.statusToggleMarkup?.("class", item.id, Boolean(item.status)) || `<button type="button" class="status-toggle ${item.status ? "is-active" : ""}" data-status-toggle data-status-entity="class" data-status-id="${item.id}" data-status="${item.status ? 1 : 0}" aria-pressed="${item.status ? "true" : "false"}"><span class="status-toggle-label">${item.status ? "ON" : "OFF"}</span><span class="status-toggle-knob"></span></button>`}</td><td class="text-center"><button onclick="classesPage.openEditModal(${item.id})" class="btn btn-primary btn-sm"><i class="ti ti-pencil icon"></i>Edit</button> ${(Number(item.groups_count ?? 0) + Number(item.enrollments_count ?? 0) + Number(item.enrollment_history_count ?? 0) + Number(item.graduations_count ?? 0)) > 0 ? `<button class="btn btn-outline-danger btn-sm" disabled title="This class is linked to other data"><i class="ti ti-lock icon"></i>Delete</button>` : `<button onclick="classesPage.deleteClass(${item.id})" class="btn btn-danger btn-sm"><i class="ti ti-trash icon"></i>Delete</button>`}</td></tr>`,
                  )
                  .join("")
