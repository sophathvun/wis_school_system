...
<body>
    @if (auth()->check())
        ...
        @endphp
        <script>
            window.userPermissions = @json($permissionCodes);
            window.currentUserId = @json($permissionUser->id);
        </script>
    @endif
    @if (!auth()->check())
        @yield('content')
        ...
        (() => {
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const normalizeSearches = () => document.querySelectorAll('input').forEach((input) => {
                    if (input.dataset.searchNormalized || input.closest('.input-icon') || input.closest(
                            '.location-combobox')) return;
                    const isSearch = input.id.toLowerCase().includes('search') || (input.placeholder ||
