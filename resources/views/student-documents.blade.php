        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const studentLabel = item =>
                    `${item.student_id || item.student_no || ''} - ${item.full_name_en || item.full_name_kh || ''}`;
                ...
                const studentCombo = $('student-document-combobox'),
                    studentMenu = studentCombo.querySelector('.location-combobox-menu'),
                    studentSearch = studentCombo.querySelector('.location-combobox-search'),
                    studentResults = studentCombo.querySelector('.location-combobox-results'),
                    studentSelected = studentCombo.querySelector('.location-combobox-selected');
                const renderStudentResults = () => {
                    const term = studentSearch.value.toLowerCase().trim();
                    const options = Array.from(studentSelect.options).slice(1).filter(option => !term || option
                ...
                const syncStudentSelection = () => {
                    studentSelected.textContent = studentSelect.value ? studentSelect.selectedOptions[0]
                        ?.textContent || 'Student' : 'Student';
                };
                ...
                studentSelect.addEventListener('change', syncStudentSelection);
                ...
                const documentTypeCombo = $('student-document-type-combobox'),
                    documentTypeMenu = documentTypeCombo.querySelector('.location-combobox-menu'),
                    documentTypeSearch = documentTypeCombo.querySelector('.location-combobox-search'),
                    documentTypeResults = documentTypeCombo.querySelector('.location-combobox-results'),
                    documentTypeSelected = documentTypeCombo.querySelector('.location-combobox-selected');
                const syncDocumentType = () => {
                    documentTypeSelected.textContent = documentTypeSelect.value ? documentTypeSelect
                        .selectedOptions[0]?.textContent || 'Document Type' : 'Document Type';
                };
                const renderDocumentTypes = () => {
                    const term = documentTypeSearch.value.toLowerCase().trim();
                    const options = Array.from(documentTypeSelect.options).slice(1).filter(option => !term || option
                        .textContent.toLowerCase().includes(term));
                ...
                const documentDropzone = $('student-document-dropzone'),
                    documentFileList = $('student-document-file-list');
                document.querySelectorAll('#studentDocumentForm .col-md-6, #studentDocumentForm .col-12').forEach(
                    wrapper => {
                        if (!wrapper.querySelector('#student-document-dropzone')) wrapper.classList.add(
                ...
                const renderDocumentFile = () => {
                    ...
                    documentFileList.innerHTML = file ?
                        `<div class="premium-document-file-item"><span class="document-file-icon"><i class="ti ti-file-description"></i></span><div class="min-w-0"><div class="document-file-name">${esc(file.name)}</div><div class="document-file-meta">${fileSize(file.size)} · Ready to upload</div></div><button type="button" class="document-file-remove" id="remove-student-document-file" aria-label="Remove file"><i class="ti ti-x"></i></button></div>` :
                        '';
                };
                ...
                const clipboardFiles = clipboard => {
                    ...
                    return Array.from(clipboard?.items || []).filter(item => item.kind === 'file').map(item => item
                        .getAsFile()).filter(Boolean);
                };
                ...
                const loadOptions = async () => {
                    Object.assign(state, await (await fetch('{{ route('student-documents.options') }}'))
                .json());
                    studentSelect.innerHTML = '<option value=""></option>' + (state.students || []).map(item =>
                        `<option value="${item.id}">${esc(studentLabel(item))}</option>`).join('');
                    documentTypeSelect.innerHTML = '<option value=""></option>' + (state.types || []).map(
                        item =>
                        `<option value="${item.id}">${esc(item.name_kh || '')}${item.name_kh ? ' / ' : ''}${esc(item.name_en)}</option>`
                        ).join('');
                ...
                const loadDocuments = async () => {
                    ...
                    if (!id) {
                        $('student-documents-table').innerHTML =
                            '<tr><td colspan="5" class="text-center">Select a student.</td></tr>';
                        return;
                    }
                    ...
                    $('student-documents-table').querySelectorAll('[data-delete]').forEach(button => button
                        .onclick = async () => {
                ...
                document.addEventListener('click', event => {
                    if (!event.target.closest('#student-document-combobox')) studentMenu.classList.add(
                    'd-none');
                    if (!event.target.closest('#student-document-type-combobox')) documentTypeMenu.classList
                        .add('d-none');
                });
                ...
                $('studentDocumentForm').onsubmit = async event => {
                    ...
                    try {
                        const response = await fetch('{{ route('student-documents.save') }}', {
                            method: 'POST',
                            headers: {
                                Accept: 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
