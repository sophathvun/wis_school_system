...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const presenceHtml = staff =>
                `<span class="staff-presence ${staff.online ? 'is-online' : 'is-offline'}"><span class="staff-presence-dot"></span>${staff.online ? 'Online' : 'Offline'}</span>`;
            ...
            const initNotificationEditor = () => {
                ...
                const syncMessage = () => {
                    editor.querySelectorAll('.is-selected').forEach(node => node.classList.remove(
                        'is-selected'));
                    textarea.value = editor.innerHTML.trim();
                    if (selectedImage) selectedImage.classList.add('is-selected');
                };
                const selectImage = image => {
                    editor.querySelectorAll('.is-selected').forEach(item => item.classList.remove(
                        'is-selected'));
                    selectedImage = image;
                ...
                const ensureResizableTables = () => {
                    editor.querySelectorAll('table[data-resizable-table]').forEach(table => {
                        ...
                        if (!cols.length && headers.length) {
                            ...
                            headers.forEach(() => colgroup.appendChild(document.createElement(
                                'col')));
                ...
                document.querySelectorAll('[data-editor-template]').forEach(button => {
                    button.addEventListener('click', () => insertHtmlAtCursor(templateHtml(button
                        .dataset.editorTemplate)));
                });
                document.querySelector('[data-editor-image]')?.addEventListener('click', () => fileInput
                ?.click());
                ...
                editor.addEventListener('paste', event => {
                    const imageFile = [...(event.clipboardData?.files || [])].find(file => file.type
                        .startsWith('image/'));
                    if (!imageFile) return;
                    event.preventDefault();
            ...
            const makeDepartmentPicker = select => {
                ...
                wrapper.innerHTML =
                    '<div class="department-selected-box is-empty" role="button" tabindex="0" aria-expanded="false"><span class="department-placeholder">Departments</span></div><div class="department-menu"><input type="search" class="form-control form-control-sm notification-picker-search" placeholder="Search departments"><div class="notification-picker-list"></div></div>';
                ...
                const syncSelected = () => {
                    const selected = [...select.selectedOptions].map(option => ({
                        value: option.value,
                    ...
                    if (selected.length) {
                        ...
                        selectedBox.innerHTML =
                            `<span class="department-box-label">Departments</span><span class="department-chip-list">${selected.map(item => `<span class="department-chip" data-dept-chip="${escapeHtml(item.value)}">${escapeHtml(item.label)}<button type="button" class="department-chip-remove" aria-label="Remove ${escapeHtml(item.label)}"><i class="ti ti-x"></i></button></span>`).join('')}</span>`;
                    } else {
                ...
                const render = () => {
                    ...
                    list.innerHTML = options.length ? options.map(option =>
                        `<label class="notification-check-row"><input class="form-check-input" type="checkbox" value="${escapeHtml(option.value)}" ${option.selected ? 'checked' : ''}><span>${escapeHtml(option.textContent)}</span></label>`
                ...
                selectedBox.addEventListener('click', event => {
                    ...
                    if (!chip) return;
                    const option = [...select.options].find(item => item.value === chip.closest(
                        '.department-chip')?.dataset.deptChip);
                ...
                selectedBox.addEventListener('click', event => {
                    ...
                    selectedBox.setAttribute('aria-expanded', menu.classList.contains('is-open') ?
                        'true' : 'false');
            ...
            const makeStaffPicker = select => {
                ...
                wrapper.innerHTML =
                    '<div class="staff-selected-box"></div><input type="search" class="form-control form-control-sm notification-picker-search" placeholder="Search staff by name, username, email, or position"><div class="notification-picker-list"></div>';
                const selectedBox = wrapper.querySelector('.staff-selected-box');
                const search = wrapper.querySelector('input');
                const list = wrapper.querySelector('.notification-picker-list');
                const syncSelected = () => {
                    const selected = [...select.selectedOptions].map(option => staffProfiles[option.value])
                        .filter(Boolean);
                    selectedBox.innerHTML = selected.length ? selected.map(staff =>
                            `<div class="staff-chip" data-staff-chip="${escapeHtml(staff.id)}">${avatarHtml(staff)}<span class="staff-meta"><span class="staff-name">${escapeHtml(staff.name)}</span><span class="staff-position">${escapeHtml(staff.position)}</span>${presenceHtml(staff)}</span><button type="button" class="staff-chip-remove" aria-label="Remove ${escapeHtml(staff.name)}"><i class="ti ti-x"></i></button></div>`
                            ).join('') :
                        '<div class="staff-selected-empty">Selected staff will appear here.</div>';
                };
                const render = () => {
                    const term = search.value.toLowerCase().trim();
                    const options = [...select.options].filter(option => {
                        ...
                        const haystack =
                            `${staff.name || ''} ${staff.username || ''} ${staff.email || ''} ${staff.position || ''}`
                            .toLowerCase();
                ...
                selectedBox.addEventListener('click', event => {
                    ...
                    const option = [...select.options].find(item => item.value === chip.dataset
                        .staffChip);
                    if (option) option.selected = false;
                    select.dispatchEvent(new Event('change', {
                        bubbles: true
            ...
            const syncDepartmentRecipients = () => {
                ...
                const selectedDepartments = [...departmentSelect.options].filter(option => option.selected).map(
