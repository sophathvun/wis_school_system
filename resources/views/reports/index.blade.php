...
@if(in_array($type, ['student-statistics', 'student-statistics-detail'], true))<div class="{{ $type === 'student-statistics-detail' ? 'col-md-3' : 'col-md-4' }} report-filter-field"><label class="form-label">Academic Year</label><div class="report-filter-combobox" data-target="reportAcademicYearValue"><button type="button" class="report-filter-toggle"><span>{{ $academicYears->firstWhere('id',(int)($filters['academic_year_id']??0))?->academic_year ?: 'All Academic Years' }}</span><i class="ti ti-chevron-down"></i></button><div class="report-filter-menu"><input type="search" class="form-control report-filter-search" placeholder="Search Academic Year"><div class="report-filter-options"><button type="button" data-value="">All Academic Years</button>@foreach($academicYears as $year)<button type="button" data-value="{{ $year->id }}">{{ $year->academic_year }}</button>@endforeach</div></div></div><input type="hidden" name="academic_year_id" id="reportAcademicYearValue" value="{{ $filters['academic_year_id']??'' }}"></div><div class="{{ $type === 'student-statistics-detail' ? 'col-md-3' : 'col-md-4' }} report-filter-field"><label class="form-label">Campus</label><div class="report-filter-combobox" data-target="reportCampusValue"><button type="button" class="report-filter-toggle"><span>{{ $campuses->firstWhere('id',(int)($filters['campus_id']??0))?->campus_name_en ?: 'All Campuses' }}</span><i class="ti ti-chevron-down"></i></button><div class="report-filter-menu"><input type="search" class="form-control report-filter-search" placeholder="Search Campus"><div class="report-filter-options"><button type="button" data-value="">All Campuses</button>@foreach($campuses as $campus)<button type="button" data-value="{{ $campus->id }}">{{ $campus->campus_name_en }}</button>@endforeach</div></div></div><input type="hidden" name="campus_id" id="reportCampusValue" value="{{ $filters['campus_id']??'' }}"></div>@if($type === 'student-statistics-detail')<div class="col-md-3 report-filter-field"><label class="form-label">Report Month</label><input type="month" name="month" class="form-control" value="{{ $filters['month'] ?? now()->format('Y-m') }}"></div>@endif @elseif($type === 'attendance-list')<div class="col-md-4 report-filter-field"><label class="form-label">Academic Year</label><div class="report-filter-combobox" data-target="reportAcademicYearValue"><button type="button" class="report-filter-toggle"><span>{{ $academicYears->firstWhere('id',(int)($filters['academic_year_id']??0))?->academic_year ?: 'All Academic Years' }}</span><i class="ti ti-chevron-down"></i></button><div class="report-filter-menu"><input type="search" class="form-control report-filter-search" placeholder="Search Academic Year"><div class="report-filter-options"><button type="button" data-value="">All Academic Years</button>@foreach($academicYears as $year)<button type="button" data-value="{{ $year->id }}">{{ $year->academic_year }}</button>@endforeach</div></div></div><input type="hidden" name="academic_year_id" id="reportAcademicYearValue" value="{{ $filters['academic_year_id']??'' }}"></div><div class="col-md-4 report-filter-field"><label class="form-label">Campus</label><div class="report-filter-combobox" data-target="reportCampusValue"><button type="button" class="report-filter-toggle"><span>{{ $campuses->firstWhere('id',(int)($filters['campus_id']??0))?->campus_name_en ?: 'All Campuses' }}</span><i class="ti ti-chevron-down"></i></button><div class="report-filter-menu"><input type="search" class="form-control report-filter-search" placeholder="Search Campus"><div class="report-filter-options"><button type="button" data-value="">All Campuses</button>@foreach($campuses as $campus)<button type="button" data-value="{{ $campus->id }}">{{ $campus->campus_name_en }}</button>@endforeach</div></div></div><input type="hidden" name="campus_id" id="reportCampusValue" value="{{ $filters['campus_id']??'' }}"></div><div class="col-md-4 report-filter-field"><label class="form-label">Grade</label><div class="report-filter-combobox" data-target="reportGradeClassValue"><button type="button" class="report-filter-toggle"><span>{{ data_get(collect($gradeClassOptions)->firstWhere('value',$filters['grade_class']??''), 'label', 'All Grades') }}</span><i class="ti ti-chevron-down"></i></button><div class="report-filter-menu"><input type="search" class="form-control report-filter-search" placeholder="Search Grade"><div class="report-filter-options"><button type="button" data-value="">All Grades</button>@foreach($gradeClassOptions as $option)<button type="button" data-value="{{ $option['value'] }}">{{ $option['label'] }}</button>@endforeach</div></div></div><input type="hidden" name="grade_class" id="reportGradeClassValue" value="{{ $filters['grade_class']??'' }}"></div><div class="col-md-4"><label class="form-label">Month</label><input name="month" type="month" class="form-control" value="{{ $filters['month'] }}"></div>@else<div class="col-md-4"><label class="form-label">Academic Year</label><select name="academic_year_id" class="form-select"><option value="">All Academic Years</option>@foreach($academicYears as $year)<option value="{{ $year->id }}" @selected(($filters['academic_year_id']??'')==$year->id)>{{ $year->academic_year }}</option>@endforeach</select></div><div class="col-md-4"><label class="form-label">Campus</label><select name="campus_id" class="form-select"><option value="">All Campuses</option>@foreach($campuses as $campus)<option value="{{ $campus->id }}" @selected(($filters['campus_id']??'')==$campus->id)>{{ $campus->campus_name_en }}</option>@endforeach</select></div><div class="col-md-4"><label class="form-label">Grade</label><select name="grade_id" class="form-select"><option value="">All Grades</option>@foreach($grades as $grade)<option value="{{ $grade->id }}" @selected(($filters['grade_id']??'')==$grade->id)>{{ $grade->grade }}</option>@endforeach</select></div><div class="col-md-4"><label class="form-label">Class ID</label><input name="class_id" type="number" min="1" class="form-control" value="{{ $filters['class_id']??'' }}"></div>@endif @if($type==='score-list')<div class="col-md-4"><label class="form-label">Score Columns</label><input name="score_columns" type="number" min="1" max="12" class="form-control" value="{{ $filters['score_columns'] }}"></div>@endif
@endif
@if(($type === 'student-list' || $type === 'student-contact-list' || $type === 'attendance-list'))<div class="row g-3 mt-2 report-print-options">@if($type !== 'attendance-list')<div class="col-md-3"><label class="form-label">Print Format</label><div class="report-native-select"><select name="print_format" class="form-select report-select-with-arrow"><option value="internal" @selected(($filters['print_format']??'internal')==='internal')>{{ $type === 'student-contact-list' ? 'Internal Student Contact List' : 'Internal Student List' }}</option><option value="moeys" @selected(($filters['print_format']??'')==='moeys')>MoEYS Khmer List</option></select><i class="ti ti-chevron-down report-native-select-arrow"></i></div></div>@endif<div class="col-md-3"><label class="form-label">Print Scope</label><div class="report-native-select"><select name="print_scope" class="form-select report-select-with-arrow"><option value="selected_class" @selected(($filters['print_scope']??'selected_class')==='selected_class')>Selected Class</option><option value="selected_classes" @selected(($filters['print_scope']??'')==='selected_classes')>Selected Classes</option><option value="all_classes" @selected(($filters['print_scope']??'')==='all_classes')>All Classes by Campus</option></select><i class="ti ti-chevron-down report-native-select-arrow"></i></div></div>@if($type !== 'attendance-list')<div class="col-md-3"><label class="form-label">Report Date</label><input type="date" name="report_date" class="form-control" value="{{ $filters['report_date'] ?? now()->format('Y-m-d') }}"></div>@endif<div class="col-md-3 report-classes-to-print-field"><label class="form-label">Classes to Print</label><select name="print_grade_classes[]" class="form-select" multiple size="3">@foreach($gradeClassOptions as $option)<option value="{{ $option['value'] }}" @selected(in_array($option['value'], $filters['print_grade_classes']??[], true))>{{ $option['label'] }}</option>@endforeach</select><div class="form-hint">Use Ctrl/Cmd to select multiple classes.</div></div></div>@endif
</div></div></form></div>
<div class="card"><div class="card-header d-flex align-items-center justify-content-between gap-2 report-preview-header"><h3 class="card-title mb-0">REPORT PREVIEW</h3>@if(($type === 'student-list' || $type === 'student-contact-list'))<div class="report-summary-card"><div class="report-summary-item report-summary-total"><div class="report-summary-label">Total Students</div><div class="report-summary-number">{{ number_format($studentSummary['total'] ?? 0) }}</div><div class="report-summary-gender">F: {{ number_format($studentSummary['total_female'] ?? 0) }} <span>|</span> M: {{ number_format($studentSummary['total_male'] ?? 0) }}</div></div><div class="report-summary-divider"></div><div class="report-summary-item report-summary-new"><div class="report-summary-label">New Students</div><div class="report-summary-number">{{ number_format($studentSummary['new_total'] ?? 0) }}</div><div class="report-summary-gender">F: {{ number_format($studentSummary['new_female'] ?? 0) }} <span>|</span> M: {{ number_format($studentSummary['new_male'] ?? 0) }}</div></div></div>@endif<div class="report-preview-actions d-flex gap-2 ms-auto"><a class="btn btn-outline-primary report-print-link" target="_blank" href="{{ route('reports.show',$type) . '?' . http_build_query($filters) }}"><i class="ti ti-printer me-1"></i>Print</a><a class="btn btn-outline-success report-excel-link" href="{{ route('reports.excel',$type) . '?' . http_build_query($filters) }}"><i class="ti ti-file-spreadsheet me-1"></i>Excel</a><a class="btn btn-outline-danger report-pdf-link" href="{{ route('reports.pdf',$type) . '?' . http_build_query($filters) }}"><i class="ti ti-file-type-pdf me-1"></i>PDF</a></div></div><div class="card-body report-preview-body">@if(($type === 'student-list' || $type === 'student-contact-list') && ($hasMorePreviewRows ?? false))<div class="alert alert-info mb-3">Showing first {{ $previewLimit }} students for fast preview. Print and Excel include all matching students.</div>@endif<div class="table-responsive">@include('reports._table')</div></div></div></div></div>
    @vite('resources/js/reportsIndex.js')
@if(($type === 'student-list' || $type === 'student-contact-list'))@endif
<style>
    .reports-tabs-header {
        align-items: center;
