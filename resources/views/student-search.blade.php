...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function makeSearchable(select, placeholder) {
        ...
        const render = () => { const term=combo.input.value.toLowerCase(); const opts=Array.from(select.options).slice(1).filter(o=>o.text.toLowerCase().includes(term)); combo.results.innerHTML='<button type="button" class="location-combobox-option" data-value="">'+combo.placeholder+'</button>'+(opts.length ? opts.map(o=>'<button type="button" class="location-combobox-option" data-value="'+o.value+'">'+esc(o.text)+'</button>').join('') : '<div class="text-secondary px-2 py-2">No results found</div>'); combo.results.querySelectorAll('[data-value]').forEach(btn=>btn.onclick=()=>{ select.value=btn.dataset.value; select.dispatchEvent(new Event('change')); combo.menu.classList.add('d-none'); }); };
    ...
    async function loadOptions() {
        const params = new URLSearchParams(); if (year.value) params.set('academic_year_id', year.value); if (campus.value) params.set('campus_id', campus.value); if (cls.value) params.set('class_id', cls.value);
    ...
    function render(data) { results.innerHTML = data.data.length ? data.data.map(item => { const s=item.student||{}; const status=item.enrollment_status||'Active'; const statusClass=status.toLowerCase()==='pending'?'bg-warning-lt':status.toLowerCase()==='active'?'bg-green-lt':'bg-secondary-lt'; return '<tr><td>' + esc(s.student_no) + '</td><td>' + esc(s.student_id) + '</td><td>' + esc(s.full_name_en || '-') + '</td><td>' + esc(item.academic_year?.academic_year || '-') + '</td><td>' + esc(item.campus?.campus_name_en || '-') + '</td><td>' + esc(item.school_class?.class_name || '-') + '</td><td>' + esc(item.school_group?.group_name || '-') + '</td><td><span class="badge '+statusClass+'">' + esc(status) + '</span></td></tr>'; }).join('') : '<tr><td colspan="8" class="text-center text-secondary py-5">No students found.</td></tr>'; summary.textContent = data.total ? 'Showing ' + data.from + ' to ' + data.to + ' of ' + data.total : ''; pagination.innerHTML = ''; for (let i=1;i<=data.last_page;i++) { const li=document.createElement('li'); li.className='page-item' + (i===data.current_page?' active':''); li.innerHTML='<button type="button" class="page-link">'+i+'</button>'; li.querySelector('button').onclick=()=>load(i); pagination.appendChild(li); } }
    ...
    [year,campus,cls,group].forEach(select => select.addEventListener('change', async function () { if (select === year) { campus.value=''; cls.value=''; group.value=''; } else if (select === campus) { cls.value=''; group.value=''; } else if (select === cls) { group.value=''; } if (select !== group) await loadOptions(); [year,campus,cls,group].forEach(item => { const combo = combos[item.id]; if (combo) combo.label.textContent = item.value ? item.selectedOptions[0]?.textContent || combo.placeholder : combo.placeholder; }); load(1); }));
