    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const year = document.getElementById('search-year'),
                campus = document.getElementById('search-campus'),
                cls = document.getElementById('search-class'),
                group = document.getElementById('search-group'),
            ...
            const optionText = item => item.campus_name_en ? item.campus_name_en : (item.class_name || item
                .group_name || item.academic_year);

            function fill(select, items, empty) {
                select.innerHTML = '<option value="">' + empty + '</option>' + items.map(item => '<option value="' +
                    item.id + '">' + esc(optionText(item)) + '</option>').join('');
            }
            const combos = {};
            ...
            function makeSearchable(select, placeholder) {
                ...
                wrapper.innerHTML =
                    '<button type="button" class="location-combobox-toggle"><span class="location-combobox-selected">' +
                    placeholder +
                    '</span><i class="ti ti-chevron-down"></i></button><div class="location-combobox-menu d-none"><input type="search" class="form-control location-combobox-search" placeholder="Search"><div class="location-combobox-results"></div></div>';
                ...
                const render = () => {
                    ...
                    const opts = Array.from(select.options).slice(1).filter(o => o.text.toLowerCase().includes(
                        term));
                    combo.results.innerHTML =
                        '<button type="button" class="location-combobox-option" data-value="">' + combo
                        .placeholder + '</button>' + (opts.length ? opts.map(o =>
                                '<button type="button" class="location-combobox-option" data-value="' + o
                                .value + '">' + esc(o.text) + '</button>').join('') :
                            '<div class="text-secondary px-2 py-2">No results found</div>');
                    combo.results.querySelectorAll('[data-value]').forEach(btn => btn.onclick = () => {
                ...
                select.addEventListener('change', () => {
                    ...
                    combo.label.textContent = selected?.value ? selected.text : combo.placeholder;
                });
            }
            ...
            function queryUrl(page) {
                ...
                if (year.value) p.set('academic_year_id', year.value);
                if (campus.value) p.set('campus_id', campus.value);
                if (cls.value) p.set('class_id', cls.value);
                if (group.value) p.set('group_id', group.value);
                if (text.value.trim()) p.set('search', text.value.trim());
                return '{{ route('searchStudent.fetch') }}?' + p;
            }

            function render(data) {
                results.innerHTML = data.data.length ? data.data.map(item => {
                        const s = item.student || {};
                        const status = item.enrollment_status || 'Active';
                        const statusClass = status.toLowerCase() === 'pending' ? 'bg-warning-lt' : status
                            .toLowerCase() === 'active' ? 'bg-green-lt' : 'bg-secondary-lt';
                        return '<tr><td>' + esc(s.student_no) + '</td><td>' + esc(s.student_id) + '</td><td>' +
                            esc(s.full_name_en || '-') + '</td><td>' + esc(item.academic_year?.academic_year ||
                                '-') + '</td><td>' + esc(item.campus?.campus_name_en || '-') + '</td><td>' +
                            esc(item.school_class?.class_name || '-') + '</td><td>' + esc(item.school_group
                                ?.group_name || '-') + '</td><td><span class="badge ' + statusClass + '">' +
                            esc(status) + '</span></td></tr>';
                    }).join('') :
                    '<tr><td colspan="8" class="text-center text-secondary py-5">No students found.</td></tr>';
                summary.textContent = data.total ? 'Showing ' + data.from + ' to ' + data.to + ' of ' + data.total :
                    '';
            ...
            async function load(page) {
                    ...
                    render(await fetch(queryUrl(page)).then(r => r.json()));
                }
                [year, campus, cls, group].forEach(select => select.addEventListener('change', async function() {
                    if (select === year) {
                        campus.value = '';
                        cls.value = '';
                        group.value = '';
                    ...
                    [year, campus, cls, group].forEach(item => {
                        ...
                        if (combo) combo.label.textContent = item.value ? item.selectedOptions[
                            0]?.textContent || combo.placeholder : combo.placeholder;
                    });
                    load(1);
                }));
            let timer;
