...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (!table) return;
            const normalize = () => table.querySelectorAll('td:nth-child(11) .badge:not(.workflow-status-button)')
                .forEach(badge => {
                    if (!badge.textContent.includes('Pending')) return;
            ...
            new MutationObserver(normalize).observe(table, {
                childList: true,
                subtree: true
            });
            normalize();
            table.addEventListener('click', async event => {
                ...
                const confirmation = window.schoolShowConfirm ? await window.schoolShowConfirm(
                    'Confirm Principal Approval',
                    'Confirm that the School Principal has signed the printed withdrawal form? This will officially withdraw the student.',
                    'Approve Withdrawal', 'Cancel') : {
                ...
                const paperResponse = await fetch(button.dataset.updateUrl + '/principal-approved', {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({
                        paper_signed: true
                ...
                const approveResponse = await fetch(button.dataset.updateUrl + '/approve', {
                    method: 'POST',
                    headers
                });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const table = document.getElementById('withdrawal-table');
            if (!table) return;
            const convertPrincipalApproved = () => table.querySelectorAll(
                'td:nth-child(11) .badge:not(.workflow-status-button)').forEach(badge => {
            ...
            new MutationObserver(convertPrincipalApproved).observe(table, {
                childList: true,
                subtree: true
            });
            ...
            table.addEventListener('click', async event => {
                ...
                const liveResponse = await fetch(button.dataset.updateUrl, {
                    headers: {
                        Accept: 'application/json'
                    }
                });
                ...
                if (!['pending', 'principal_approved'].includes(liveStatus)) {
                    if (window.Swal) await window.Swal.fire({
                        title: liveStatus === 'cancelled' ? 'Request Cancelled' : liveStatus ===
                            'rejected' ? 'Request Rejected' : 'Withdrawal Completed',
                        text: liveStatus === 'cancelled' ?
                            'This request has already been cancelled.' : liveStatus ===
                ...
                const status = button.dataset.status || (button.textContent.includes('Pending') ?
                    'pending' : 'principal_approved');
                ...
                if (choice.value === 'reject' || choice.value === 'cancel') {
                    const reasonResult = await window.Swal.fire({
                        title: choice.value === 'reject' ? 'Reject Withdrawal Request' :
                            'Cancel Withdrawal Request',
                        input: 'textarea',
                        inputLabel: choice.value === 'reject' ? 'Rejection reason' :
                            'Cancellation reason',
                        inputValidator: value => !value?.trim() ? 'A reason is required.' :
                            undefined,
                        showCancelButton: true,
                        confirmButtonText: choice.value === 'reject' ? 'Reject Request' :
                            'Cancel Request',
                ...
                const endpoint = choice.value === 'approve' ? '/approve' : choice.value === 'reject' ?
                    '/reject' : '/cancel';
                if (choice.value === 'approve' && status === 'pending') {
                    const paperResponse = await fetch(button.dataset.updateUrl +
                    '/principal-approved', {
                        method: 'POST',
                        headers,
                        body: JSON.stringify({
                ...
                const response = await fetch(button.dataset.updateUrl + endpoint, {
                    method: 'POST',
                    headers,
                    body: JSON.stringify(choice.value === 'reject' ? {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const syncWorkflowLabels = () => table.querySelectorAll('tr').forEach(async row => {
                ...
                if (!['pending', 'principal_approved'].includes(status) && badge.classList.contains(
                        'workflow-status-button')) {
                    const replacement = document.createElement('span');
                    replacement.className = 'badge';
                    replacement.textContent = labels[status] || status;
                    replacement.classList.add(...(status === 'approved' ? ['bg-success-lt',
                        'text-success'
                    ] : status === 'rejected' ? ['bg-danger-lt', 'text-danger'] : [
                ...
                badge.classList.remove('bg-warning-lt', 'text-warning', 'bg-success-lt', 'text-success',
                    'bg-danger-lt', 'text-danger', 'bg-secondary-lt', 'text-secondary');
                const classes = status === 'approved' ? ['bg-success-lt', 'text-success'] : status ===
                    'rejected' ? ['bg-danger-lt', 'text-danger'] : status === 'cancelled' ? [
                        'bg-secondary-lt', 'text-secondary'
                    ] : ['bg-warning-lt', 'text-warning'];
                badge.classList.add(...classes);
            });
            new MutationObserver(syncWorkflowLabels).observe(table, {
                childList: true,
                subtree: true
            });
            syncWorkflowLabels();
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('withdrawalModal')),
            ...
            reasonPanel.innerHTML =
                '<label class="form-label">Reason for Withdrawal * <small class="text-secondary">(select all that apply)</small></label><div class="row g-2">' +
                (@json(\App\Http\Controllers\StudentWithdrawalController::REASONS)).map(reason =>
                    '<div class="col-md-6"><label class="form-check"><input class="form-check-input withdrawal-reason-check" type="checkbox" value="' +
            ...
            const syncReasons = () => {
                ...
                const otherEn = field('withdrawal-other-en')?.value.trim() || '',
                    otherKh = field('withdrawal-other-kh')?.value.trim() || '';
                const en = selected.map(item => item.dataset.en).concat(otherEn ? [otherEn] : []).filter(
                        Boolean),
                    kh = selected.map(item => item.dataset.kh).concat(otherKh ? [otherKh] : []).filter(Boolean);
                field('withdrawal-reason-en').value = en.join('; ');
                field('withdrawal-reason-kh').value = kh.join('; ');
            ...
            document.addEventListener('change', event => {
                if (event.target.matches('.withdrawal-reason-check,#withdrawal-other-check')) {
                    field('withdrawal-other-fields')?.classList.toggle('d-none', !field(
                        'withdrawal-other-check')?.checked);
                    syncReasons();
                }
            });
            ...
            ['withdrawal-year-search', 'withdrawal-campus-search', 'withdrawal-grade-class-search',
                'withdrawal-student-search'
            ].forEach(id => {
            ...
            const fill = (id, items, label, empty) => {
                field(id).innerHTML = '<option value="">' + empty + '</option>' + items.map(item =>
                    '<option value="' + item.id + '">' + esc(item[label]) + '</option>').join('');
            };
            const filterSelect = (selectId, searchId) => {
                const select = field(selectId),
                    term = (field(searchId)?.value || '').toLowerCase();
                Array.from(select.options).forEach((option, index) => {
                    option.hidden = index > 0 && !option.text.toLowerCase().includes(term);
                });
            };
            const setupSearchableSelect = (id, label) => {
                ...
                const menu = field(id + '-menu'),
                    searchInput = field(id + '-menu-search'),
            ...
            document.addEventListener('click', event => {
                if (!event.target.closest('.withdrawal-search-combobox')) document.querySelectorAll(
                    '.withdrawal-search-combobox .location-combobox-menu').forEach(menu => menu
                    .classList.add('d-none'));
            });
            const studentName = item =>
                `${item.student?.student_id||item.student?.student_no||''} - ${item.student?.full_name_en||''}`;
            ...
            const historyPhoto = student => student?.photo_path ? '<img src="/storage/' + esc(student.photo_path) +
                '" alt="Student photo" class="withdrawal-history-photo">' :
                '<span class="withdrawal-history-photo withdrawal-history-photo-placeholder bg-secondary-lt"><i class="ti ti-user"></i></span>';
            ...
            const updateGradeClasses = (items) => {
                ...
                field('withdrawal-class').value = '';
                field('withdrawal-grade-class').dispatchEvent(new Event('change'));
            };
            const refreshDependentOptions = async () => {
                if (!field('withdrawal-year').value) {
                ...
                const result = await fetch('{{ route('student-withdrawals.options') }}?' + params
                ...
                if ((result.campuses || []).some(item => String(item.id) === String(currentCampus))) field(
                    'withdrawal-campus').value = currentCampus;
                else field('withdrawal-campus').value = '';
                updateGradeClasses(field('withdrawal-campus').value ? result.gradeClasses || [] : []);
                renderStudentSelect();
            };
            const hasAllStudentFilters = () => Boolean(field('withdrawal-year').value && field('withdrawal-campus')
                .value && field('withdrawal-grade').value && field('withdrawal-class').value);
            ...
            const renderStudents = async () => {
                ...
                const rows = (await fetch(studentQuery()).then(response => response.json())).enrollments ||
                    [];
                const list = field('withdrawal-students');
                list.innerHTML = rows.length ? rows.map(item =>
                        '<label class="form-check mb-1"><input class="form-check-input withdrawal-student" type="checkbox" value="' +
                        item.id + '"><span class="form-check-label">' + esc(studentName(item)) +
                        '</span></label>').join('') :
                    '<div class="text-secondary small">No active students found.</div>';
            };
            const renderStudentSelect = async () => {
                ...
                const result = await fetch(studentQuery()).then(response => response.json());
                const enrollments = result.enrollments || [];
                window.withdrawalEnrollmentFamilies = Object.fromEntries(enrollments.map(item => [String(
                    item.id), item.family_members || []]));
                field('withdrawal-enrollment-id').innerHTML = '<option value="">Select Student</option>' +
                    enrollments.map(item => '<option value="' + item.id + '" data-family-members="' + esc(
                            JSON.stringify(item.family_members || [])) + '">' + esc(studentName(item)) +
                        '</option>').join('');
                field('withdrawal-enrollment-id').dispatchEvent(new Event('change'));
            };
            const updateType = () => {
            ...
            const gradeCode = grade => {
                ...
                if (value.toLowerCase() === 'nursery') return 'N';
                return value.replace(/^Grade\s+/i, '');
            };
            const loadOptions = async () => {
                options = await fetch('{{ route('student-withdrawals.options') }}').then(r => r.json());
                fill('withdrawal-campus', [], 'campus_name_en', 'Select Campus');
                ...
                field('withdrawal-grade-class').onchange = () => {
                    ...
                    field('withdrawal-grade').value = gradeId || '';
                    field('withdrawal-class').value = classId || '';
                    renderStudentSelect();
                    renderStudents();
                };
                clearStudentResults();
            };
            field('withdrawal-type').onchange = updateType;
            field('withdrawal-year').onchange = refreshDependentOptions;
            field('withdrawal-campus').onchange = refreshDependentOptions;
            field('withdrawal-student-search').classList.add('d-none');
            field('withdrawal-student-search').oninput = renderStudentSelect;
            [
                ['withdrawal-year', 'withdrawal-year-search'],
                ['withdrawal-campus', 'withdrawal-campus-search'],
                ['withdrawal-grade-class', 'withdrawal-grade-class-search']
            ].forEach(([selectId, searchId]) => field(searchId).oninput = () => filterSelect(selectId, searchId));
            document.getElementById('newWithdrawal').onclick = async () => {
                form.reset();
                (window.withdrawalDatePicker?.setValue || ((value) => field('withdrawal-date').value =
            ...
            form.onsubmit = async event => {
                ...
                const type = field('withdrawal-type').value;
                const common = {
                    withdrawal_date: field('withdrawal-date').value,
                    reason: field('withdrawal-reason').value,
                    notes: field('withdrawal-notes').value
                };
                ...
                if (type === 'student') {
                    ...
                    payload = {
                        ...common,
                        enrollment_id: field('withdrawal-enrollment-id').value
                    };
                } else {
                    payload = {
                        ...common,
                        from_campus_id: field('withdrawal-campus').value,
                        from_academic_year_id: field('withdrawal-year').value,
                        from_grade_id: field('withdrawal-grade').value,
                        from_class_id: field('withdrawal-class').value
                    };
                    if (type === 'selected') {
                        endpoint = 'withdraw-selected';
                ...
                const response = await fetch('{{ url('/student-withdrawals') }}/' + endpoint, {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
                ...
                if (!response.ok) {
                    field('withdrawal-error').textContent = result.message || Object.values(result.errors ||
                    {})[0]?.[0] || 'Unable to withdraw student.';
                    field('withdrawal-error').classList.remove('d-none');
                    return;
                }
                modal.hide();
                loadHistory();
            ...
            const historyFilters = [{
                    ...
                    label: 'Academic Year',
                    container: 'withdrawal-history-year-filter',
                    value: item => item.id,
                    text: item => item.academic_year
                },
                {
                    key: 'campus_id',
                    label: 'Campus',
                    container: 'withdrawal-history-campus-filter',
                    value: item => item.id,
                    text: item => item.campus_name_en
                },
                {
                    key: 'grade_class',
                    label: 'Grade + Class',
                ...
                },
                {
                    key: 'session_id',
                    label: 'Group',
                    container: 'withdrawal-history-group-filter',
                    value: item => item.id,
                    text: item => item.session_short_name || item.session_name
                },
                {
                    key: 'student_id',
                    label: 'Student Name',
                    container: 'withdrawal-history-student-filter',
                    value: item => item.id,
                    text: item => item.full_name_en || ''
                },
            ];
            let historySort = 'date',
                historyDirection = 'desc',
                historyPerPage = 10;
            const historySelects = {};
            const setupHistoryFilter = (config, items) => {
                const container = field(config.container);
                if (!container) return;
                const selectId = config.container + '-select';
                container.innerHTML = '<select id="' + selectId + '" class="d-none"><option value="">All ' +
                    config.label + '</option>' + (items || []).map(item => '<option value="' + esc(config.value(
                        item)) + '">' + esc(config.text(item)) + '</option>').join('') +
                    '</select><div class="location-combobox withdrawal-history-combobox"><button type="button" class="location-combobox-toggle"><span class="location-combobox-selected">All ' +
                    config.label +
                    '</span><i class="ti ti-chevron-down"></i></button><div class="location-combobox-menu d-none"><input type="search" class="form-control location-combobox-search" placeholder="Search ' +
                    config.label + '"><div class="location-combobox-results"></div></div></div>';
                const select = field(selectId),
                    combo = container.querySelector('.withdrawal-history-combobox'),
                    toggle = combo.querySelector('.location-combobox-toggle'),
                    menu = combo.querySelector('.location-combobox-menu'),
                    search = combo.querySelector('.location-combobox-search'),
                    results = combo.querySelector('.location-combobox-results'),
                    selected = combo.querySelector('.location-combobox-selected');
                historySelects[config.key] = select;
                const render = () => {
                    const term = search.value.toLowerCase().trim();
                    const options = Array.from(select.options).slice(1).filter(option => !term || option
                        .textContent.toLowerCase().includes(term));
                    const allOption =
                        '<button type="button" class="location-combobox-option" data-value="">All ' + config
                        .label + '</button>';
                    results.innerHTML = allOption + (options.length ? options.map(option =>
                            '<button type="button" class="location-combobox-option" data-value="' + esc(
                                option.value) + '">' + esc(option.textContent) + '</button>').join('') :
                        '<div class="text-secondary px-2 py-2">No options found</div>');
                };
                toggle.onclick = event => {
                    event.stopPropagation();
                    document.querySelectorAll('.withdrawal-history-combobox .location-combobox-menu')
                        .forEach(other => {
                            if (other !== menu) other.classList.add('d-none');
                        });
                    menu.classList.toggle('d-none');
                ...
                results.onclick = event => {
                    ...
                    if (!option) return;
                    select.value = option.dataset.value;
                    selected.textContent = select.value ? select.selectedOptions[0].textContent : 'All ' +
                        config.label;
                    menu.classList.add('d-none');
                    refreshHistoryOptions(config.key).then(() => loadHistory(1));
                };
            };
            const historyOptionKey = key => key === 'academic_year_id' ? 'academicYears' : key === 'campus_id' ?
                'campuses' : key === 'grade_class' ? 'gradeClasses' : key === 'session_id' ? 'groups' : 'students';
            const refreshHistoryOptions = async changedKey => {
                ...
                const currentValues = Object.fromEntries(order.map(key => [key, historySelects[key]
                    ?.value || ''
                ]));
                order.slice(changedIndex + 1).forEach(key => {
                    if (historySelects[key]) historySelects[key].value = '';
                    currentValues[key] = '';
                });
                const params = new URLSearchParams();
                if (currentValues.academic_year_id) params.set('academic_year_id', currentValues
                    .academic_year_id);
                if (currentValues.campus_id) params.set('campus_id', currentValues.campus_id);
                if (currentValues.grade_class) {
                    const [gradeId, classId] = currentValues.grade_class.split('|');
                    params.set('grade_id', gradeId);
                    params.set('class_id', classId);
                }
                if (currentValues.session_id) params.set('session_id', currentValues.session_id);
                const data = await fetch('{{ route('student-withdrawals.history-options') }}?' + params
                    .toString()).then(response => response.json());
                historyFilters.forEach(config => {
                    const value = currentValues[config.key] || '';
                    setupHistoryFilter(config, data[historyOptionKey(config.key)] || []);
                    const select = historySelects[config.key];
                    if (value && Array.from(select.options).some(option => option.value ===
                        value)) {
                        select.value = value;
                        const display = field(config.container)?.querySelector(
                            '.location-combobox-selected');
                        if (display) display.textContent = select.selectedOptions[0].textContent;
                    }
                });
            };
            document.addEventListener('click', event => {
                if (!event.target.closest('.withdrawal-history-combobox')) document.querySelectorAll(
                    '.withdrawal-history-combobox .location-combobox-menu').forEach(menu => menu
                    .classList.add('d-none'));
            });
            const historyParams = page => {
                const params = new URLSearchParams({
                    page,
                    perPage: historyPerPage,
                    search: field('withdrawal-search').value,
                    sortBy: historySort,
                    sortDir: historyDirection
                });
                Object.entries(historySelects).forEach(([key, select]) => {
                    if (!select?.value) return;
                    if (key === 'grade_class') {
                        const [gradeId, classId] = select.value.split('|');
                        params.set('grade_id', gradeId);
                        params.set('class_id', classId);
                    } else params.set(key, select.value);
                });
                return params;
            };
            const renderHistoryPagination = result => {
                const pagination = field('withdrawal-pagination');
                if (!pagination) return;
                const current = result.current_page || 1,
                    last = result.last_page || 1;
                const pages = last <= 5 ? Array.from({
                    length: last
                }, (_, index) => index + 1) : current <= 3 ? [1, 2, 3, '…', last] : current >= last - 2 ? [
                ...
                pagination.innerHTML =
                    ...
                    '"><i class="ti ti-chevron-left"></i></a></li>' + pages.map(page => page === '…' ?
                    ...
                    '"><a class="page-link withdrawal-page" href="#" data-page="' + (current + 1) +
                    '"><i class="ti ti-chevron-right"></i></a></li></ul><p class="premium-pagination-info m-0">Showing <strong>' +
                    (result.from || 0) + ' to ' + (result.to || 0) + '</strong> of <strong>' + (result.total ||
                        0) +
                    ' entries</strong></p><div class="premium-pagination-controls"><label class="premium-pagination-select"><select class="form-select form-select-sm"><option value="10" ' +
                    (historyPerPage === 10 ? 'selected' : '') + '>10 / page</option><option value="25" ' + (
                        historyPerPage === 25 ? 'selected' : '') + '>25 / page</option><option value="50" ' + (
                        historyPerPage === 50 ? 'selected' : '') + '>50 / page</option><option value="100" ' + (
                        historyPerPage === 100 ? 'selected' : '') +
                    '>100 / page</option></select></label></div></div>';
                pagination.querySelectorAll('.withdrawal-page').forEach(button => button.onclick = event => {
                    event.preventDefault();
                    const page = Number(button.dataset.page);
                    if (page >= 1 && page <= last && page !== current) loadHistory(page);
                });
                pagination.querySelector('select')?.addEventListener('change', event => {
                    historyPerPage = Number(event.target.value);
                    loadHistory(1);
                });
            };
            const loadHistory = async (page = 1) => {
                const result = await fetch('{{ route('student-withdrawals.fetch') }}?' + historyParams(
                ...
                field('withdrawal-table').innerHTML = result.data?.length ? result.data.map((item,
                    index) => {
                        ...
                        const gradeClass = gradeCode(item.grade?.grade) + (item.school_class
                            ?.class_name || '');
                        ...
                        return '<tr><td>' + esc((result.from || 1) + index) + '</td><td>' +
                            historyPhoto(student) + '</td><td class="text-nowrap">' + esc(
                                historyStudentId(student)) +
                            '</td><td class="withdrawal-history-name"><div class="school-profile-khmer text-secondary small">' +
                            esc(nameKh || '-') + '</div><div>' + esc(nameEn || '-') +
                            '</div><div class="withdrawal-history-phone text-secondary">' + esc(student
                                .home_phone || '-') + '</div></td><td>' + esc(item.academic_year
                                ?.academic_year || '-') + '</td><td><div class="fw-semibold">' + esc(
                                gradeClass || '-') +
                            '</div><div class="withdrawal-history-track text-secondary">' + esc(track ||
                                '-') + '</div></td><td>' + esc(item.session?.session_short_name || item
                                .session?.session_name || '-') +
                            '</td><td class="withdrawal-history-date">' + esc(historyDate(item
                                .effective_on)) + '</td><td><div class="fw-semibold">' + esc(
                                requestedName) + '</div><div class="text-secondary small">' + esc(
                                requestedPhone || '-') + '</div></td><td>' + esc(item.changed_by
                                ?.name || 'System') + '</td><td><span class="badge ' + statusClass +
                            '">' + esc(statusLabel) +
                            '</span></td><td class="text-nowrap"><div class="d-inline-flex gap-1"><button type="button" class="btn btn-sm btn-outline-secondary edit-withdrawal" data-edit-url="' +
                            esc(item.edit_url || '#') + '" data-update-url="' + esc(item.update_url ||
                                '#') +
                            '" title="Edit"><i class="ti ti-edit"></i></button><a class="btn btn-sm btn-outline-primary" href="' +
                            esc(item.form_url || '#') +
            ...
            document.querySelectorAll('[data-withdrawal-sort]').forEach(button => button.onclick = () => {
                const selected = button.dataset.withdrawalSort;
                historyDirection = historySort === selected && historyDirection === 'asc' ? 'desc' : 'asc';
                historySort = selected;
                loadHistory(1);
            });
            let searchTimer;
            field('withdrawal-search').oninput = () => {
                clearTimeout(searchTimer);
                searchTimer = setTimeout(() => loadHistory(1), 300);
            };
            fetch('{{ route('student-withdrawals.history-options') }}').then(response => response.json()).then(
                data => {
                    historyFilters.forEach(config => setupHistoryFilter(config, data[config.key ===
                        ...
                        'campuses' : config.key === 'grade_class' ? 'gradeClasses' : config.key ===
                        'session_id' ? 'groups' : 'students'] || []));
                    loadHistory();
                }).catch(() => loadHistory());
        });
    </script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (reasonHeading) {
                reasonHeading.outerHTML =
                    '<h5 class="mb-3">REASONS FOR WITHDRAWAL: <small class="text-secondary fw-normal">(Select all that apply)</small></h5>';
            }
            ...
            document.getElementById('withdrawal-date')?.closest('.col-md-6')?.classList.add(
            ...
            fetch('{{ route('student-withdrawals.options') }}').then(response => response.json()).then(data => {
                ...
                reasonRow.innerHTML = activeReasons.map(reason =>
                        '<div class="col-md-6"><label class="form-check"><input class="form-check-input withdrawal-reason-check" type="checkbox" value="' +
                        escapeHtml(reason.key) + '" data-en="' + escapeHtml(reason.en) + '" data-kh="' +
                        escapeHtml(reason.kh) +
                        '"><span class="form-check-label"><span class="d-block school-profile-khmer">' +
                        escapeHtml(reason.kh) + '</span>' + escapeHtml(reason.en) + '</span></label></div>')
                    .join('') +
                    '<div class="col-md-6"><label class="form-check"><input class="form-check-input" type="checkbox" id="withdrawal-other-check"><span class="form-check-label">Other</span></label></div>';
            });
            ...
            const ensureRequestedMembers = async () => {
                ...
                const result = await fetch('{{ url('/student-withdrawals/enrollments') }}/' + selectedId +
                    '/family', {
                        headers: {
                            Accept: 'application/json'
                        }
            ...
            const getRequestedMember = type => getRequestedMembers().find(member => member.relationship_type ===
                type) || null;
            const setRequestedCardData = () => {
                ['mother', 'father', 'guardian'].forEach(type => {
                    ...
                    if (field('withdrawal-' + type + '-phone')) field('withdrawal-' + type + '-phone')
                        .textContent = phone || '-';
                    if (type === 'guardian') {
                        if (field('withdrawal-guardian-preview-name')) field(
                                'withdrawal-guardian-preview-name').textContent = name ||
                            'Manual / existing guardian';
                        if (field('withdrawal-guardian-preview-phone')) field(
                            'withdrawal-guardian-preview-phone').textContent = phone || 'Editable';
                    }
                });
            };
            const applyRequestedBy = async type => {
                ...
                if (field('withdrawal-requested-by-type')) field('withdrawal-requested-by-type').value =
                    type || '';
            ...
            document.addEventListener('change', event => {
                if (event.target.matches('#withdrawal-requested-by-type')) void applyRequestedBy(event
                    .target.value);
                if (event.target.matches('#withdrawal-enrollment-id')) {
                    ...
                    setTimeout(() => void applyRequestedBy(field('withdrawal-requested-by-type')?.value ||
                        ''), 0);
                }
            });
            document.addEventListener('click', event => {
            ...
            const syncReasons = () => {
                ...
                const en = selected.map(item => item.dataset.en).concat(otherEn ? [otherEn] : []).filter(
                    Boolean);
                const kh = selected.map(item => item.dataset.kh).concat(otherKh ? [otherKh] : []).filter(
                    Boolean);
            ...
            form.onsubmit = async event => {
                ...
                const common = {
                    ...
                    reasons: Array.from(document.querySelectorAll('.withdrawal-reason-check:checked'))
                        .map(input => input.value),
                    reason_kh: value('withdrawal-reason-kh'),
                    other_reason_en: value('withdrawal-other-en'),
                    other_reason_kh: value('withdrawal-other-kh'),
                    new_school: value('withdrawal-new-school'),
                    new_school_address: value('withdrawal-new-school-address'),
                ...
                if (type === 'student') {
                    ...
                    payload = {
                        ...common,
                        enrollment_id: value('withdrawal-enrollment-id')
                    };
                } else {
                    payload = {
                        ...common,
                        from_campus_id: value('withdrawal-campus'),
                        from_academic_year_id: value('withdrawal-year'),
                        from_grade_id: value('withdrawal-grade'),
                        from_class_id: value('withdrawal-class')
                    };
                    if (type === 'selected') {
                        endpoint = 'withdraw-selected';
                        payload.enrollment_ids = Array.from(document.querySelectorAll(
                ...
                if (!response.ok) {
                    ...
                    error.textContent = result.message || Object.values(result.errors || {})[0]?.[0] ||
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const formatWithdrawalIso = date => date ?
            ...
            const setupWithdrawalDatePicker = prefix => {
                ...
                const hidden = document.getElementById(prefix.replace(/_/g, '-') === 'withdrawal-date' ?
                    'withdrawal-date' : 'edit-withdrawal-date') || document.getElementById(prefix);
                ...
                const renderCalendar = () => {
                    ...
                    if (label) label.textContent = cursor.toLocaleDateString('en-US', {
                        month: 'long',
                        year: 'numeric'
                    });
                    renderYears();
                    ...
                    while (cells.length < 42) cells.push(new Date(year, month + 1, cells.length - start -
                        count + 1));
                    days.innerHTML = cells.map(date => {
                        const iso = formatWithdrawalIso(date);
                        const outside = date.getMonth() !== month ? ' is-outside' : '';
                        const active = selected && iso === formatWithdrawalIso(selected) ?
                            ' is-selected' : '';
                ...
                trigger.addEventListener('click', event => {
                    event.stopPropagation();
                    const selected = parseWithdrawalDate(hidden.value);
                    cursor = selected ? new Date(selected.getFullYear(), selected.getMonth(), 1) :
                ...
                document.addEventListener('click', event => {
                    if (!picker.contains(event.target)) popup.classList.add('d-none');
                });
                return {
                    setValue
                };
            ...
            const heading = document.querySelector(
                '.withdrawal-reason-panel > .form-label, .withdrawal-reason-panel > h5');
            if (heading) heading.outerHTML =
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const getEditMember = type => editFamilyMembers.find(member => member.relationship_type === type) ||
                null;
            ...
            const checkedReasons = () => Array.from(document.querySelectorAll(
                '.edit-withdrawal-reason-check:checked'));
            const syncEditReasons = () => {
                ...
                const data = {
                    reason: selected.map(input => input.dataset.en).concat(otherEn ? [otherEn] : []).filter(
                        Boolean).join('; '),
                    reason_kh: selected.map(input => input.dataset.kh).concat(otherKh ? [otherKh] : [])
                        .filter(Boolean).join('; '),
                };
            ...
            document.addEventListener('click', async event => {
                ...
                form.reset();
                document.querySelectorAll('.edit-other-field').forEach(item => item.classList.add(
                    'd-none'));
                ...
                principalApprovalButton.classList.toggle('d-none', data.withdrawal_status !==
                'pending');
                approveButton.classList.toggle('d-none', data.withdrawal_status !==
                    'principal_approved');
                rejectButton.classList.toggle('d-none', !['pending', 'principal_approved'].includes(data
                    .withdrawal_status));
                cancelButton.classList.toggle('d-none', !['pending', 'principal_approved', 'rejected']
                    .includes(data.withdrawal_status));
                ...
                field('edit-student-id').textContent = studentSummary.student_id ? 'Student ID: ' +
                ...
                const isGrade12 = String(studentSummary.grade || '').replace(/^Grade\s*/i, '')
                .trim() === '12';
                const contextParts = [
                ...
                (window.editWithdrawalDatePicker?.setValue || ((value) => field('edit-withdrawal-date')
                    .value = value))(String(data.effective_on || '').slice(0, 10));
                ...
                document.querySelectorAll('.edit-withdrawal-reason-check').forEach(input => input
                    .checked = (data.reasons || []).includes(input.value));
                field('edit-other-check').checked = Boolean(data.other_reason_en || data
                    .other_reason_kh);
                document.querySelectorAll('.edit-other-field').forEach(item => item.classList.toggle(
            ...
            document.getElementById('mark-principal-approved')?.addEventListener('click', async () => {
                const confirmation = window.schoolShowConfirm ? await window.schoolShowConfirm(
                    'Confirm Principal Approval',
                    'Confirm that the School Principal has signed the printed withdrawal form?',
                    'Record Approval', 'Cancel') : {
                    isConfirmed: window.confirm(
                        'Confirm that the School Principal has signed the printed withdrawal form?'
                        )
                };
                if (!confirmation.isConfirmed) return;
                const response = await fetch(form.dataset.updateUrl + '/principal-approved', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
                ...
                if (!response.ok) {
                    const error = field('edit-withdrawal-error');
                    error.textContent = result.message || 'Unable to record paper approval.';
                    error.classList.remove('d-none');
                    return;
                }
                window.location.reload();
            ...
            document.getElementById('approve-withdrawal')?.addEventListener('click', async () => {
                const confirmation = window.schoolShowConfirm ? await window.schoolShowConfirm(
                    'Approve Withdrawal', 'The student will become officially withdrawn.',
                    'Approve Withdrawal', 'Cancel') : {
                    isConfirmed: window.confirm(
                        'Approve this withdrawal? The student will become officially withdrawn.'
                        )
                };
                if (!confirmation.isConfirmed) return;
                const response = await fetch(form.dataset.updateUrl + '/approve', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                if (!response.ok) {
                    const error = field('edit-withdrawal-error');
                    error.textContent = result.message || 'Unable to approve withdrawal.';
                    error.classList.remove('d-none');
                    return;
                }
                window.location.reload();
            ...
            document.getElementById('reject-withdrawal')?.addEventListener('click', async () => {
                const result = window.Swal ? await window.Swal.fire({
                    ...
                    inputValidator: value => !value?.trim() ?
                        'A rejection reason is required.' : undefined,
                    didOpen: popup => popup.querySelector('textarea')?.focus(),
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    isConfirmed: window.confirm('Reject this withdrawal request?'),
                    value: 'Rejected by administrator'
                };
                if (!result.isConfirmed) return;
                const response = await fetch(form.dataset.updateUrl + '/reject', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
            ...
            document.getElementById('cancel-withdrawal')?.addEventListener('click', async () => {
                const result = window.Swal ? await window.Swal.fire({
                    ...
                    inputValidator: value => !value?.trim() ?
                        'A cancellation reason is required.' : undefined,
                    didOpen: popup => popup.querySelector('textarea')?.focus(),
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonText: 'Cancel Request',
                    confirmButtonColor: '#6c7a91',
                    cancelButtonText: 'Keep Request',
                    reverseButtons: true
                }) : {
                    isConfirmed: window.confirm('Cancel this withdrawal request?'),
                    value: 'Cancelled by administrator'
                };
                if (!result.isConfirmed) return;
                const response = await fetch(form.dataset.updateUrl + '/cancel', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
            ...
            document.addEventListener('change', event => {
                if (event.target.matches('#edit-other-check')) {
                    document.querySelectorAll('.edit-other-field').forEach(item => item.classList.toggle(
                        'd-none', !event.target.checked));
                    syncEditReasons();
                }
                if (event.target.matches('#edit-requested-by-type')) applyEditRequestedBy(event.target
                    .value);
            ...
            form.addEventListener('submit', async event => {
                ...
                const payload = {
                    ...
                    other_reason_en: field('edit-other-check').checked ? field('edit-other-en')
                        .value : '',
                    other_reason_kh: field('edit-other-check').checked ? field('edit-other-kh')
                        .value : '',
                ...
                if (!response.ok) {
                    const error = field('edit-withdrawal-error');
                    error.textContent = result.message || Object.values(result.errors || {})[0]?.[0] ||
                        'Unable to update withdrawal form.';
                    error.classList.remove('d-none');
                    return;
                }
                modal.hide();
