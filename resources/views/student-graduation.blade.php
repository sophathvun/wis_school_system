        ...
        (() => {
            window.addEventListener('load', () => {
                ...
                const gradeClassLabel = (item = {}) => {
                    ...
                    const cls = String(item.school_class?.class_name || '').replace(/^grade\s*/i, '')
                .trim();
                    return cls && grade && cls.toLowerCase().startsWith(grade.toLowerCase()) ? cls :
                        `${grade}${cls}` || '-';
                };
                ...
                const classOptionLabel = (item = {}) => {
                    ...
                    return cls && grade && cls.toLowerCase().startsWith(grade.toLowerCase()) ?
                        `Grade ${cls}` : `Grade ${grade}${cls}` || cls || '-';
                };
                ...
                const fill = (id, items, textKey, allLabel = '') => {
                    ...
                    select.innerHTML = `<option value="">${esc(allLabel)}</option>` + (items || []).map((
                        item) => `<option value="${item.id}">${esc(label(item))}</option>`).join('');
                ...
                const renderSearchable = (id) => {
                    ...
                    const matches = Array.from(item.select.options).slice(1).filter((option) => option
                        .textContent.toLowerCase().includes(term));
                    const allOption = ['graduation-year', 'graduation-campus', 'graduation-class'].includes(
                            id) ?
                        `<button type="button" class="location-combobox-option" data-value="">All ${id === 'graduation-year' ? 'Academic Years' : id === 'graduation-campus' ? 'Campuses' : 'Grades'}</button>` :
                        '';
                    item.results.innerHTML = allOption + (matches.length ? matches.slice(0, 60).map((
                        option) =>
                        `<button type="button" class="location-combobox-option" data-value="${option.value}">${esc(option.textContent)}</button>`
                ...
                const makeSearchable = (id) => {
                    ...
                    wrapper.innerHTML =
                        `<button type="button" class="location-combobox-toggle"><span class="location-combobox-selected"></span><i class="ti ti-chevron-down"></i></button><div class="location-combobox-menu d-none"><input type="search" class="form-control location-combobox-search" placeholder="Search"><div class="location-combobox-results"></div></div>`;
                ...
                const refreshSearchableValue = (id) => {
                    ...
                    item.selected.textContent = selected?.value ? selected.textContent : (id ===
                        'graduation-year' ? 'All Academic Years' : id === 'graduation-campus' ?
                        'All Campuses' : id === 'graduation-class' ? 'All Grades' : '');
                };
                const setupSearchables = () => ['graduation-year', 'graduation-campus', 'graduation-class',
                    'batch-year', 'batch-campus', 'batch-class'
                ].forEach(makeSearchable);

                const enrollmentOptions = () => Array.from(field('graduation-enrollment').options);
                const selectedEnrollmentOptions = () => enrollmentOptions().filter((option) =>
                    selectedGraduateEnrollmentIds.has(option.value));
                const refreshEnrollmentMultiValue = () => {
                    if (!enrollmentMulti) return;
                    const selected = selectedEnrollmentOptions();
                    enrollmentMulti.selected.textContent = selected.length ?
                        `${selected.length} student${selected.length > 1 ? 's' : ''} selected` : '';
                    field('graduation-current').innerHTML = selected.length ?
                        selected.map((option) =>
                            `<span class="graduation-selected-pill">${esc(option.textContent)} <button type="button" data-remove-enrollment="${option.value}" aria-label="Remove student">&times;</button></span>`
                            ).join('') :
                        '';
                    field('graduation-current').querySelectorAll('[data-remove-enrollment]').forEach((
                        button) => {
                        button.onclick = () => {
                            selectedGraduateEnrollmentIds.delete(button.dataset
                                .removeEnrollment);
                            const option = enrollmentOptions().find((item) => item.value ===
                                button.dataset.removeEnrollment);
                ...
                const renderEnrollmentMultiOptions = () => {
                    ...
                    const matches = enrollmentOptions().filter((option) => option.textContent.toLowerCase()
                        .includes(term) || (option.dataset.info || '').toLowerCase().includes(term));
                    enrollmentMulti.results.innerHTML = matches.length ? matches.slice(0, 120).map((
                        option) => `
            ...
            `).join('') : '<div class="text-secondary px-2 py-2">No results found</div>';
                ...
                const loadOptions = async (list = false, params = {}) => {
                    ...
                    const response = await fetch(`/student-graduations/options?${query.toString()}`, {
                        headers: {
                ...
                const loadListOptions = async () => {
                    ...
                    fill('graduation-year', data.academicYears || [], 'academic_year',
                        'All Academic Years');
                ...
                const refreshCampuses = async (yearId, list, targetId) => {
                    const data = await loadOptions(list, {
                ...
                const updateSortIcons = () => {
                    document.querySelectorAll('[data-graduation-sort-icon]').forEach((icon) => {
                        icon.textContent = icon.dataset.graduationSortIcon === graduationSortBy ? (
                            graduationSortDir === 'asc' ? '↑' : '↓') : '';
                    });
                    document.querySelectorAll('[data-graduation-sort]').forEach((button) => {
                        button.classList.toggle('text-primary', button.dataset.graduationSort ===
                            graduationSortBy);
                    });
                };
                const render = async (page = 1, pageSize = Number(field('graduation-per-page').value ||
                10)) => {
                    currentPage = page;
                    const query = new URLSearchParams({
                        page,
                    ...
                    const response = await fetch(`/student-graduations/fetch?${query.toString()}`, {
                        headers: {
                            Accept: 'application/json'
                        }
                    });
                    ...
                    field('graduationTable').innerHTML = rows.length ? rows.map((item, index) => {
                        ...
                        '<tr><td colspan="12" class="text-center">No graduated students found.</td></tr>';
                ...
                const refreshEnrollmentOptions = async () => {
                    ...
                    field('graduation-enrollment').innerHTML = (data.enrollments || []).map((item) => {
                        ...
                        const info =
                            `${item.academic_year?.academic_year || '-'} | ${item.campus?.campus_name_en || '-'} | Class ${item.school_class?.class_name || '-'} | Group ${item.session?.session_short_name || '-'}`;
                ...
                const applyScope = () => {
                    ...
                    document.querySelectorAll('.graduation-student-field').forEach((element) => element
                        .classList.toggle('d-none', batch));
                ...
                field('graduation-year').onchange = async () => {
                    await refreshCampuses(field('graduation-year').value, true, 'graduation-campus');
                    await refreshClasses(field('graduation-campus').value, field('graduation-year')
                        .value, 'graduation-class', true);
                    await render(1);
                };
                field('graduation-campus').onchange = async () => {
                    await refreshClasses(field('graduation-campus').value, field('graduation-year')
                        .value, 'graduation-class', true);
                    await render(1);
                };
                ...
                document.querySelectorAll('[data-graduation-sort]').forEach((button) => button.addEventListener(
                    'click', () => {
                        if (graduationSortBy === button.dataset.graduationSort) graduationSortDir =
                            graduationSortDir === 'asc' ? 'desc' : 'asc';
                        else {
                            graduationSortBy = button.dataset.graduationSort;
                            graduationSortDir = 'asc';
                        }
                        render(1);
                    }));
                ...
                field('certificate-number').oninput = () => {
                    field('certificate-number').value = field('certificate-number').value.replace(/\D/g, '')
                        .slice(0, 4);
                };
                field('batch-campus').onchange = () => refreshClasses(field('batch-campus').value, field(
                    'batch-year').value, 'batch-class', false);
                field('batch-year').onchange = async () => {
                    await refreshCampuses(field('batch-year').value, false, 'batch-campus');
                    await refreshClasses(field('batch-campus').value, field('batch-year').value,
                        'batch-class', false);
                };
                ...
                field('graduationForm').onsubmit = async (event) => {
                    ...
                    if (!/^\d{4}$/.test(certificatePrefix)) {
                        field('graduationError').textContent =
                            'Please enter exactly 4 digits for the certificate prefix.';
                        field('graduationError').classList.remove('d-none');
                        return;
                    }
                    const scope = field('graduation-scope').value;
                    const payload = {
                        graduation_date: field('graduation-date').value,
                        certificate_number: certificatePrefix,
                        is_alumni: field('is-alumni').checked ? 1 : 0,
                        notes: field('graduation-notes').value
                    };
                    ...
                    if (scope === 'student') {
                        ...
                        if (!enrollmentIds.length) {
                            field('graduationError').textContent =
                                'Please select at least one Grade 12 student.';
                            field('graduationError').classList.remove('d-none');
                            return;
                        }
                        ...
                        payload.scope = scope;
                        payload.academic_year_id = field('batch-year').value;
                        payload.campus_id = scope === 'all_campuses' ? '' : field('batch-campus').value;
                        payload.class_id = scope === 'class' ? field('batch-class').value : '';
                    }
                    const response = await fetch(`/student-graduations/${endpoint}`, {
                        method: 'POST',
                        headers: {
                            Accept: 'application/json',
                    ...
                    if (!response.ok) {
                        field('graduationError').textContent = result.message || Object.values(result
                            .errors || {})[0]?.[0] || 'Unable to graduate student.';
                        field('graduationError').classList.remove('d-none');
                        return;
                    }
                    modal.hide();
                ...
                loadListOptions().then(() => render()).catch(() => {
                    field('graduationTable').innerHTML =
                        '<tr><td colspan="12" class="text-center text-danger">Unable to load graduation records.</td></tr>';
                });
            });
        })();
    </script>
