...
class PositionManagementController
{
    ...
    public function index(Request $request)
    {
        ...
        $sortBy = in_array($request->query('sortBy', 'name'), ['name', 'department', 'code', 'status'], true)
        ...
        $positions = Position::with('department')
            ...
            ->when(
                $sortBy === 'department',
                fn ($query) => $query->orderBy(Department::select('name')->whereColumn('access_departments.id', 'access_positions.department_id'), $sortDir),
    ...
    public function print(Request $request)
    {
        ...
        return view('positions-pdf', [
            'positions' => $this->positionReportRows(),
            'logoSrc' => $this->positionReportLogoSrc(),
            'printMode' => true,
        ]);
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        $filename = 'position-list-' . now('Asia/Phnom_Penh')->format('Ymd-His') . '.xlsx';
        $path = $this->positionExcelPath($this->positionReportRows());
    ...
    public function delete(Request $request, Position $position)
    {
        ...
        if ($position->users()->exists()) {
            return back()->withErrors(['position' => 'This position cannot be deleted because it is assigned to users.']);
        }
    ...
    private function positionReportRows()
    {
    ...
    private function positionReportLogoSrc(): ?string
    {
        $logoPath = $this->positionReportLogoPath();
    ...
    private function positionReportLogoPath(): ?string
    {
    ...
    private function positionExcelPath($positions): string
    {
        $path = tempnam(sys_get_temp_dir(), 'position-list-');
        $xlsxPath = $path . '.xlsx';
        ...
        $entries = [
            ...
            'xl/worksheets/sheet1.xml' => $this->positionWorksheetXml($positions, (bool) $logoPath),
        ];
    ...
    private function positionWorksheetXml($positions, bool $hasLogo): string
    {
        $lastRow = $positions->count() + 9;
        $rows = [
            $this->xlsxRow(1, [], 28),
            $this->xlsxRow(2, [], 28),
            $this->xlsxRow(3, [], 28),
            $this->xlsxRow(4, [], 10),
            $this->xlsxRow(5, [['A', 'តារាងមុខតំណែង', 1]], 30),
            $this->xlsxRow(6, [['A', 'Position List', 2]], 26),
        ...
        foreach ($positions as $index => $position) {
            $rows[] = $this->xlsxRow($index + 10, [
                ['A', (string) ($index + 1), 5],
                ['B', $position->name ?: '-', 5],
                ['C', $position->department?->name ?: '-', 5],
                ['D', $position->code ?: '-', 5],
                ['E', $position->status ? 'Active' : 'Inactive', 5],
            ], 22);
        }
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            . '<cols><col min="1" max="1" width="8" customWidth="1"/><col min="2" max="2" width="30" customWidth="1"/><col min="3" max="3" width="30" customWidth="1"/><col min="4" max="4" width="18" customWidth="1"/><col min="5" max="5" width="16" customWidth="1"/></cols>'
            . '<sheetData>' . implode('', $rows) . '</sheetData>'
            . '<mergeCells count="5"><mergeCell ref="A1:E3"/><mergeCell ref="A5:E5"/><mergeCell ref="A6:E6"/><mergeCell ref="A7:E7"/><mergeCell ref="A8:E8"/></mergeCells>'
    ...
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Position List" sheetId="1" r:id="rId1"/></sheets></workbook>';
    }
